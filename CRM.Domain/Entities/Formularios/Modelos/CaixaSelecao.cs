...
public sealed class CaixaSelecao : PerguntaObjetiva
{
    private CaixaSelecao(Guid id,
                        string enunciado,
                        string descricao,
                        bool obrigatorio,
                        int ordem)
    ...
    public static CaixaSelecao Build(Guid id,
                                     string enunciado,
                                     string descricao,
                                     bool obrigatorio,
                                     int ordem)
    ...
    public override bool IsRespostaValida(Resposta resposta)
    {
        ...
        int quantidadeDeAlternativasValidas = Alternativas.Count;
        ...
        if (Obrigatorio)
        {
            if (quantidadeDeAlternativasSelecionadas > 0 &&
                quantidadeDeAlternativasSelecionadas <= quantidadeDeAlternativasValidas)
            {
                return true;
            }
